...
def consultas_medico(request):
    ...
    if request.method == "GET":
        ...
        print(status_filtrar)
        status_code = False
        if status_filtrar:
            for status_completo, codigo in status_dict.items():
                if str(status_filtrar).lower() in status_completo.lower():
                    status_code = codigo
            if status_code != False:
                consultas_hoje = consultas_hoje.filter(status=status_code)
                consultas_restantes = consultas_restantes.filter(status=status_code)
        
        data_filtrar =  request.GET.get("data")
        if data_filtrar:
            data_filtrar = data_filtrar
